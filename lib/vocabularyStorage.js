...
// Safe Firestore background sync helper
async function syncVocabToFirestore(key, value) {
  if (typeof window === 'undefined') return;
  try {
    ...
    if (isConfigured && auth?.currentUser && db) {
      ...
      const docRef = doc(db, 'users', auth.currentUser.uid, 'data', 'vocabulary');
      await setDoc(docRef, { [key]: value, updatedAt: new Date().toISOString() }, { merge: true });

      // Automatically sync updated diligence XP stats to leaderboard
    ...
    console.warn('Vocab Firestore sync warning:', err);
  }
}

let activeVocabUnsubscribe = null;

/**
 * Clears local vocabulary tracking data
 */
export function clearVocabStorage() {
  if (typeof window === 'undefined') return;
  localStorage.removeItem(STORAGE_KEYS.MASTERED_WORDS);
  localStorage.removeItem(STORAGE_KEYS.STARRED_WORDS);
  localStorage.removeItem(STORAGE_KEYS.QUIZ_HISTORY);
  localStorage.removeItem(STORAGE_KEYS.LAST_TOPIC);
}

/**
 * Initializes realtime vocabulary progress synchronization with Cloud Firestore
 * @param {Function} onDataChange - callback({ masteredIds, starredIds, quizHistory, lastTopic })
 * @returns {Function} unsubscribe
 */
export async function initVocabStorage(onDataChange) {
  if (typeof window === 'undefined') return () => {};
  ...
  try {
    ...
    if (isConfigured && auth && db) {
      ...
      const authUnsub = onAuthStateChanged(auth, (user) => {
        if (activeVocabUnsubscribe) {
          activeVocabUnsubscribe();
          activeVocabUnsubscribe = null;
        }

        if (user) {
          const docRef = doc(db, 'users', user.uid, 'data', 'vocabulary');
          activeVocabUnsubscribe = onSnapshot(docRef, (snap) => {
            if (snap.exists()) {
              const remote = snap.data();
              const masteredIds = remote.masteredIds || [];
              const starredIds = remote.starredIds || [];
              const quizHistory = remote.quizHistory || [];
              const lastTopic = remote.lastTopic || 'school_supplies';

              setStoredJson(STORAGE_KEYS.MASTERED_WORDS, masteredIds);
              setStoredJson(STORAGE_KEYS.STARRED_WORDS, starredIds);
              setStoredJson(STORAGE_KEYS.QUIZ_HISTORY, quizHistory);
              localStorage.setItem(STORAGE_KEYS.LAST_TOPIC, lastTopic);

              if (onDataChange) {
                onDataChange({ masteredIds, starredIds, quizHistory, lastTopic });
              }
            } else {
              // Brand new user: clear local vocab
              clearVocabStorage();
              if (onDataChange) {
                onDataChange({ masteredIds: [], starredIds: [], quizHistory: [], lastTopic: 'school_supplies' });
              }
            }
          });
        } else {
      ...
      return () => {
        authUnsub();
        if (activeVocabUnsubscribe) activeVocabUnsubscribe();
      };
    }
  } catch (e) {
    console.warn('Vocab storage init fallback:', e);
  }
...
export function toggleMasteredWord(wordId) {
  ...
  syncVocabToFirestore('masteredIds', updated);
  return updated;
}
...
export function toggleStarredWord(wordId) {
  ...
  syncVocabToFirestore('starredIds', updated);
  return updated;
}
...
export function saveLastTopic(topicId) {
  ...
  try {
    ...
    syncVocabToFirestore('lastTopic', topicId);
  } catch (e) {
...
export function saveQuizResult(result) {
  ...
  syncVocabToFirestore('quizHistory', updated);

  // Background sync detailed session to subcollection
  if (typeof window !== 'undefined') {
    import('./firestoreService')
      .then(({ recordQuizSession }) => {
        import('./firebase').then(({ getFirebaseInstance }) => {
          const { auth, isConfigured } = getFirebaseInstance();
          if (isConfigured && auth?.currentUser) {
            recordQuizSession(auth.currentUser.uid, newEntry);
          }
        });
      })
      .catch(() => {});
